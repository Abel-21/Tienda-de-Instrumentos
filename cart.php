...
<body style="padding: 80px 0px">
    ...
    <center>
        ...
        <textarea placeholder='Escribe aquí...' name='texto' cols='80' rows='10'>
            <?php
        ...
        <p> Quitar de la lista <br>

            <form action="eliminar.php" method="post">
                <?php
                $result3 = mysqli_query($link, "SELECT * FROM instrumento JOIN pedido ON pedido.id_instrumento = instrumento.id_instrumento WHERE id_apartado='$id_apartado'");
                ?>
                <select name="quitar">
                    <?php
                    while ($row = mysqli_fetch_array($result3)) {
                        $nombre = $row['nombre'];
                        echo "<option value='" . $row['id_instrumento'] . "'>" . $row['nombre'] . "</option>";
                        //echo "<option value ='".$carrera['id']."'>".$carrera['nombre']."</option>";
                    }
                    ?>
                </select> <br> <br>
                <!-- <input type='submit' name='submit'/> -->
                <button onclick="comprobar(this.form)" type='submit' name="submit" class='btn btn-danger' type='button'>Quitar</button> <br> <br>

            </form>

            <form action="Apartar.php">
